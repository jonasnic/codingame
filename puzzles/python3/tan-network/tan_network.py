...
def distance(node1: Node, node2: Node) -> float:
    ...
    x: float = (node2.longitude - node1.longitude) * math.cos((node1.latitude + node2.latitude) / 2)
    y: float = node2.latitude - node1.latitude
...
def read_game_input() -> Graph:
    ...
    for i in range(n):
        ...
        identifier: str = stop[0]
        name: str = stop[1].replace('\"', '')
        latitude: float = math.radians(float(stop[3]))
        longitude: float = math.radians(float(stop[4]))
...
def traverse_with_a_star(graph: Graph) -> Optional[Dict[str, str]]:
    ...
    while len(open_set) != 0:
        current_id: str = compute_minimum(open_set, f_scores)
        ...
        for neighbor_id in graph.nodes[current_id].routes:
            ...
            neighbor_node: Node = graph.nodes[neighbor_id]
            g_score: float = g_scores[current_id] + distance(graph.nodes[current_id], neighbor_node)
...
def compute_minimum(open_set, f_scores) -> str:
    """Compute the node_id in open_set with the minimum f_score"""
    min_id: Optional[str] = None
    min_f_score: float = math.inf
    for identifier in open_set:
        f_score: str = f_scores[identifier]
...
def reconstruct_path(graph: Graph, came_from: Optional[Dict[str, str]]) -> List[str]:
    current_id: str = graph.end_id
    stack: List[str] = []
