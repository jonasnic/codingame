...
class Node:
    def __init__(self, x: int, y: int, terrain: str):
        ...
        self.lake_index: int = DEFAULT_INDEX
...
class Grid:
    ...
    def fill_neighbors(self, queue, node, lake_index):
        if node.x + 1 < self.width:
            right_neighbor: Node = self.nodes[node.y][node.x + 1]
            self.fill(queue, right_neighbor, lake_index)

        if node.x > 0:
            left_neighbor: Node = self.nodes[node.y][node.x - 1]
            self.fill(queue, left_neighbor, lake_index)

        if node.y + 1 < self.height:
            down_neighbor: Node = self.nodes[node.y + 1][node.x]
            self.fill(queue, down_neighbor, lake_index)

        if node.y > 0:
            up_neighbor: Node = self.nodes[node.y - 1][node.x]
